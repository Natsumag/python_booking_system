...
if page == 'users':
    ...
    to_datatime = lambda x: datetime.datetime.fromisoformat(x).strftime('%Y/%m/%d %H:%M')

    df_bookings['user_id'] = df_bookings['user_id'].map(to_username)
    df_bookings['room_id'] = df_bookings['room_id'].map(to_room_name)
    df_bookings['start_datetime'] = df_bookings['start_datetime'].map(to_datatime)
    df_bookings['end_datetime'] = df_bookings['end_datetime'].map(to_datatime)
    ...
    with st.form(key='booking'):
        ...
        book_num: int = st.number_input('予約人数', step=1, min_value=1)
        date = st.date_input('日付: ', min_value=datetime.date.today())
        start_time = st.time_input('開始時刻: ', value=datetime.time(hour=8, minute=0))
        end_time = st.time_input('開始時刻: ', value=datetime.time(hour=22, minute=0))
